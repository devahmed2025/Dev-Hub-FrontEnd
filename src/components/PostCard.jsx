...
const PostCard = memo(
  ({ post, onShare, onReport, showCommentsInitially = false }) => {
    ...
    useEffect(() => {
      setOptimisticLiked(isLiked); // Sync with server
    }, [isLiked]);
    ...
    const handleLikeToggle = useCallback(async () => {
      ...
      try {
        if (!optimisticLiked) {
          // We think it's unliked, so try to LIKE it
          await dispatch(
            likeCommunityPost({ postId: post._id, userId })
          ).unwrap();
        } else {
          // We think it's liked, so try to UNLIKE it
          await dispatch(
            unlikeCommunityPost({ postId: post._id, userId })
          ).unwrap();
        }
      } catch (err) {
        // Revert on real error (but not if it's 400 "Already liked/unliked")
        const status = err?.response?.status || err?.status;
        const isKnownToggleError = status === 400;

        if (!isKnownToggleError) {
          // Revert UI only for real errors
          setOptimisticLiked((prev) => !prev);
          const message = isArabic(post?.content)
            ? 'فشل في تحديث الإعجاب'
            : 'Failed to update like';
    ...
    }, [dispatch, post?._id, userId, optimisticLiked, post?.content]);
    ...
    return (
      ...
      >
        ...
        <div>
          <RichTextRenderer
            content={
              post.content || (contentIsArabic ? 'لا يوجد محتوى' : 'No content')
            }
